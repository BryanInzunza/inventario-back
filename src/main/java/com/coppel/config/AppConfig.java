...
public class AppConfig {
    ...
    public String compatibilityContextVersion() {
        ...
        String majorVersion = this.buildProperties.getVersion() == null ? majorVersionContext
                : this.buildProperties.getVersion().split("[.]")[0];

        if (majorVersionContext.equals(majorVersion)) {
            return this.buildProperties.getVersion();
        } else {
            return String.format("Major project version in pom.xml (%s), differs from major context version (%s)",
                    majorVersion, semanticContextVersion);
        }
    }
}
